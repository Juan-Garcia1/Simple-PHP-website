...
//WE USE TRIM TO REMOVE ANY WHITE SPACE
...
// SPECIFYING REQUIRED FIELDS
  // WE CHECK IF THE INPUTS ARE EMPTY, IF SO WE DISPLAY AN ERROR MESSAGE
  if($name == "" || $email == "" || $category == "" || $title == "") {
    $error_message =  "Please fill in the required fields: Name, Email, Category, Title";
  }
  // PREVENTS ROBOTS FROM ATTACKING
  if(!isset($error_message) && $_POST["address"] != "") {
    $error_message =  "Bad form input";
  }

// WILL BE USING A THIRD-PARTY LIBRARY PHPMAILER TO SEND EMAIL
require("Inc/phpmailer/class.phpmailer.php");
$mail = new PHPMailer;

$mail->Host = 'smtp.example.com';  // Specify main and backup SMTP servers
$mail->SMTPAuth = true;                               // Enable SMTP authentication
$mail->Username = 'user@example';                 // SMTP username
$mail->Password = 'secret';                           // SMTP password
$mail->SMTPSecure = 'tls';                            // Enable TLS encryption, `ssl` also accepted
$mail->Port = 587;


// WE WANT TO RETURN AN ERROR IF ITS NOT TRUE(EMAIL ADDRESS NOT VALID)
if(!isset($error_message) && !$mail->ValidateAddress($email)) {
$error_message =  "Invalid Email address";
}

//  WRITE A CONDITIONAL THAT ONLY EXECUTES WHEN THERE ARE NO ERRORS
if(!isset($error_message)) {

    // WORKING WITH CONCATENATION AND WHITE SPACE
    $email_body = "";
...
// IF SEND WAS SUCCESSFULL, WE'LL REDIRECT THE USER TO A THANK YOU PAGE
    if($mail->send()) {
  ...
  <div class="wrapper">
    ...
    <?php
      if(isset($_GET["status"]) && $_GET["status"] == "thanks") {
        ...
        // IF THE STATUS IS NOT SET TO THANKS WE'RE GOING TO DISPLAY THE FORM

    if(isset($error_message)) {
      echo "<p class='message'>".$error_message."</p>";
    } else {
      echo "<p>If you think theres is something I&rsquo;m missing, let me know! Complete the form to send an email</p>";
    }
     ?>

<!--ALWAYS WANT TO USE THE $_POST METHOD TO SUBMIT FORM DATA TO THE SERVER-->
    <form method="POST" action="suggest.php">
      <table>
        ...
        <tr>
          ...
          <td>
            <input type="text" id="title" name="title" value="<?php if(isset($title)) {echo htmlspecialchars($title);} ?>" />
          </td>
        </tr>
        ...
        <tr>
            ...
            <tr>
              ...
              <td>
                <input type="text" id="year" name="year" value="<?php if(isset($year)) {echo htmlspecialchars($year);} ?>" />
              </td>
            </tr>
